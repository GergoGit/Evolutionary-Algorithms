...
$f: \R^{D}\to\R^1$ is the fitness function and $fitness_i=f(x_i)$ is the fitness value of $x_i$. Parameter $\sigma$ controls the random offspring generation process.\\
\noindent
...
\end{equation}
Where $rand_{j} \in U(0, 1)$, where $U$ stands for uniform distribution. \\ 
...
\caption{Evolutionary Strategy}
 ...
 \Begin{
 Set $N$: population size, $T$: number of iterations, $k$: number of offsprings, $\sigma$: control parameter, $\delta$: decay parameter for $\sigma$ \\
 ...
 \While{$t\leq T$ or Stopping criteria not met}{
  \For{$i \gets 1 \textrm{ to } N$}{
  	  \For{$j \gets 1 \textrm{ to } k$}{
         ...
         Check search space \\
         Calculate fitess value $fitness_{ij}$\\  	  
  	  }
    }
    \If{apply elitism}{
